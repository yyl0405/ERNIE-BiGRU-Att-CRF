    ...
    for line in content:
        ...
        # 获得一句话
            ...
            if tokens:
                ...
                labels_f = ["<start>"] + labels + ['<eos>']
                input_ids = [int(vocab.get(word, vocab['[UNK]'])) for word in tokens_f]
                label_ids = [label_dic.get(tag, label_dic['<pad>']) for tag in labels_f]
...
def save_model(model, epoch, path='result/', **kwargs):
    ...
    if kwargs.get('name', None) is None:
        ...
        # name = 'eduner-ERNIE-BiLSTM-CRF' +cur_time + '--epoch=%d' % epoch
